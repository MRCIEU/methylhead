process Alignment {
    ...
    tuple val(sample_id), path("${sample_id}*_val_1_bismark_bt2_PE_report.txt"), emit: alignment_report
    ...
    script:
       ...
       def bismark_cmd = "bismark --genome_folder ${genome_folder} --nucleotide_coverage -1 ${fq[0]} -2 ${fq[1]} --bam" 
    if (u_param) {
