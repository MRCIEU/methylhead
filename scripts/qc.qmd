...
## <sup>Total Summed Read Depth</sup>

```{r}
#| echo: false
#| label: total-summed-read-depth
#| error: true
#| fig-cap: "**Figure 13**: Total summed read depth across all measured loci. This represents the total sequencing yield for each sample."

# Step 1: Define a dynamic label based on the dataset parameter.
# This makes the plot titles and labels general.
...
# Step 2: Calculate the sum of coverage depths for each sample.

reads_in_roi <- colSums(input.coverage, na.rm = TRUE)

# Step 3: Create a clean data frame for plotting.
total_cov <- tibble(
    Sample = names(reads_in_roi),
    Total_Coverage = reads_in_roi
  ) %>%
  arrange(desc(Total_Coverage))

# Step 4: Plot the total coverage using the dynamic labels.
ggplot(total_cov,
       aes(x    = reorder(Sample, Total_Coverage),
           y    = Total_Coverage,
           fill = Total_Coverage)) +
  geom_col(width = 0.8, colour = "grey20", linewidth = 0.15) +
  geom_text(aes(label = format(Total_Coverage, big.mark = ",")),
            hjust = -0.05, size = 3.3) +
  scale_fill_gradient(name = "Total Depth",
                      low  = "#440154", # deep purple
                      high = "#FDE725", # lime-yellow
                      labels = scales::label_number(scale_cut = scales::cut_short_scale())) +
  scale_y_continuous(
    labels = scales::label_number(scale_cut = scales::cut_short_scale()),
    expand = expansion(mult = c(0, 0.18))) + # More space for labels
  coord_flip(clip = "off") +
  # Use the 'roi_label' to create a dynamic title
  labs(title    = paste("Total Summed Read Depth Across", roi_label),
       subtitle = "Higher bars indicate greater overall sequencing yield",
       x        = "Sample",
       y        = "Total Coverage Depth") +
