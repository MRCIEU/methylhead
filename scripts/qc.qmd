...
<details style="margin-top:1rem;">
  ...
  <p>
    <code>methylation_matrix</code> is a sample &times; CpG table where each cell
    contains the fractional methylation (<em>&beta;</em>-value) at that locus:
    <br><code>&beta; = methylated_reads / (methylated_reads + unmethylated_reads)</code>.
  </p>

  <p><strong>Pipeline overview:</strong></p>
  <ol style="margin-left:1rem;">
    <li><strong>Adapter trimming</strong> - raw FASTQs processed with
        <code>Trim&nbsp;Galore!</code> (uses <code>cutadapt</code>).</li>
    <li><strong>Alignment</strong> - trimmed reads aligned with
        <code>bwa-meth</code>; duplicates removed using <code>Picard</code>.</li>
    ...
    <li><strong>Import to R</strong> - counts loaded with
        ...
        <code>pipeline</code> and <code>assembly</code> to suit your workflow.</li>
    <li><strong>Coverage filter</strong> - loci with coverage &lt;
        <strong>mincov = {coverage_threshold}</strong> are discarded.
        <br><small><em>mincov</em> is the minimum reads per CpG we trust;
        lower coverage blows up variance and invites false positives.</small></li>
    <li><strong>Matrix export</strong> - the <code>methylKit</code> object is
        converted to a matrix (<code>unite()</code> &rarr; <code>regionCounts()</code>)
        and saved as the <code>methylation_matrix</code>.</li>
  </ol>

  <p>
    Row count therefore equals the number of CpG loci that pass alignment,
    deduplication, and the <code>mincov</code> filter <em>before</em> any
    panel-based intersection.
  </p>
</details>
'))
```
