...
if (params$dataset == "panel") {
  ...
  setnames(panel, c("chr","start","end"), c("chr","start","end"))
  setnames(setDT(methylation_matrix),  c("chr","start","end"), c("chr","start","end"))
  setkey(panel, chr, start, end)
  setkey(setDT(methylation_matrix) ,  chr, start, end)
  panel <- foverlaps(methylation_matrix, panel, type = "within", nomatch = 0L)
  panel <- panel %>% select(-matches("chr|start|end"))
  ...
  methylation_matrix<-data.frame(methylation_matrix)
...
## <sup>Proportion of Reads Mapped Regions</sup>

```{r}
#| echo: false
#| label: Proportion of reads mapped regions
#| error: true
#| fig-cap: "**Figure 13**: Proportion of sequencing reads assigned to regions of interest for each sample."

roi_label <- if (!exists("params") || is.null(params$dataset)) {
               "selected CpG loci"
             } else if (params$dataset == "panel") {
               "targeted panel regions"
             } else {
               paste(params$dataset, "regions")
             }
# Calculate proportion of reads in ROI 
# Ensure input.coverage is a data frame with samples as columns
reads_in_roi <- colSums(input.coverage, na.rm = TRUE)

prop_roi <- 
  tibble(Sample        = names(reads_in_roi),
         Reads_in_ROI  = reads_in_roi) %>% 
  left_join(reads %>% select(Sample, Reads), by = "Sample") %>% 
  mutate(Proportion_ROI = Reads_in_ROI / Reads,
         Prop_Label     = sprintf('%.1f%%', Proportion_ROI * 100)) %>% 
  arrange(desc(Proportion_ROI))

ggplot(prop_roi,
       aes(x    = reorder(Sample, Proportion_ROI),
           y    = Proportion_ROI,
           fill = Proportion_ROI)) +
  geom_col(width = 0.8, colour = "grey20", linewidth = 0.15) +
  geom_text(aes(label = Prop_Label),
            hjust = -0.05, size = 3.3) +
  scale_fill_gradient(name = paste(roi_label, "read %"),
                      low  = "#440154",  # deep purple
                      high = "#FDE725") +# lime-yellow
  scale_y_continuous(
    labels  = function(x) sprintf('%.0f%%', x * 100),
    expand  = expansion(mult = c(0, 0.02)),
    limits  = c(0, max(prop_roi$Proportion_ROI) * 1.05)) +
  coord_flip(clip = "off") +
  labs(title    = paste("Proportion of reads mapping to", roi_label),
       subtitle = "Higher bars indicate better on-target efficiency",
       x        = "Sample",
       y        = paste("Reads in", roi_label, "(%)")) +
...
## <sup>Distribution of Read Counts in Regions</sup>

```{r}
#| echo: false
#| label: distribution of read counts in regions 1
#| error: true
#| fig-cap: "**Figure 14**: Locus-level read depth across. A log10</sub>-scaled histogram shows how many target loci fall into each coverage bin. A tall bar at the left indicates that most loci are sequenced at low depth, while the sparsity of right-hand bins reveals that very few loci achieve high coverage."
...
# Long-format coverage table 
cov_long <- as.data.frame(input.coverage)
cov_long$loc <- rownames(cov_long)                      
...
ggplot(cov_long, aes(x = Read_Count)) +
  geom_histogram(binwidth = 1,
                 colour = "grey30",
                 fill   = "#69b3a2") +
  scale_x_log10() +
  labs(x = "Reads per locus (log10)",
       y = "Count of loci") +
  theme_minimal(base_size = 11)
```
...
```{r}
#| echo: false
#| label: distribution of read counts in regions 2
#| error: true
#| fig-cap: "**Figure 15**: Distribution of locus read depths per sample. Each horizontal box-and-whisker summarises the coverage spread within a single library. Medians (bold lines) reflect typical depth, box heights show inter-quartile ranges, and outlying points highlight loci that are substantially over-or under-represented. Consistent boxes imply balanced libraries; wide or low-lying boxes flag samples that may need re-sequencing or library optimisation."

## summary per sample 
summ <- aggregate(Read_Count ~ Sample, cov_long, function(x)
                  c(med = median(x, na.rm = TRUE),
                    n   = length(x[!is.na(x)])))
summ <- data.frame(
  Sample = summ$Sample,
  med    = summ$Read_Count[ , "med"],
  n_loci = summ$Read_Count[ , "n"],
  stringsAsFactors = FALSE
)

## order samples (high-to-low median) 
ord <- summ[order(-summ$med), "Sample"]
cov_long$Sample <- factor(cov_long$Sample, levels = ord)
summ$Sample     <- factor(summ$Sample,     levels = ord)

## colour palette 
pal <- colorRampPalette(c("#3957a3", "#2aa37d", "#f4c430"))(length(ord))
names(pal) <- ord

## dynamic labels 
depth_label  <- if (!exists("params") || is.null(params$dataset)) {
                  "loci"
                } else if (params$dataset == "panel") {
                  "panel loci"
                } else {
                  paste(params$dataset, "loci")
                }

iqr_subtitle <- "Boxes = IQR; whiskers = 1.5\u00D7IQR; dots = outliers"
caption_text <- "n = number of loci   |   m = median read depth"

## plot
ggplot(cov_long, aes(x = Sample, y = Read_Count, fill = Sample)) +
  geom_boxplot(
    width          = 0.55,
    outlier.shape  = 21,
    outlier.size   = 0.6,
  ...
  labs(
    title    = sprintf("Per-sample read-depth across %s", depth_label),
    subtitle = iqr_subtitle,
    x        = NULL,
    y        = "Reads per locus (log10)",
