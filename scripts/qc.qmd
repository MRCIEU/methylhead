...
ggplot(coverage_long,
       aes(x = Sample, y = Coverage, fill = Set)) +
  geom_violin(trim = FALSE) +                        # ← değişiklik burada
  facet_wrap(~Set, ncol = 1, scales = "free_y") +
  labs(title = "Coverage per Sample: Panel & Total",
       x = "Sample", y = "Coverage") +
  scale_x_discrete(labels = function(x) sub(".*_(S\\d+)", "\\1", x)) +
...
depth_long <- bind_rows(
  ...
  mutate(
    Region_Type = factor(Region_Type, levels = c("Total_Regions", "Panel_Regions")),
    Sample      = forcats::fct_reorder(Sample, Count, .fun = max)
  )
...
ggplot(depth_long, aes(x = Count, y = Sample, fill = Region_Type)) +
  ...
  geom_text(aes(label = scales::comma(Count)),
            ...
            hjust = -0.15, size = 3) +
  scale_fill_manual(
    values = setNames(c("#4E79A7", "#E15759"),
                      c("Total_Regions", "Panel_Regions")),
    labels = c("Total reads (all sites)", "Reads in panel sites"),
    name   = NULL
  ) +
  scale_x_continuous(
...
hs %>%
  ggplot(aes(x = `Mean target coverage`, y = `Median target coverage`)) +
  geom_point(aes(color = Sample), size = 4, alpha = 0.7) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
  geom_text(
    aes(label = sub(".*_(S\\d+)", "\\1", Sample)), 
    vjust = -0.8, 
    size = 3.5, 
    color = "black",
    check_overlap = TRUE
  ...
  labs(
    title = "Mean vs. Median Target (Panel) Coverage",
    x = "Mean Target (Panel) Coverage",
    y = "Median Target (Panel) Coverage"
  ) +
