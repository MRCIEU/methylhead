...
for (path in files_path$V1) {
  fname <- basename(path)
  if (fname %in% names(label_map)) {
    assign(label_map[[fname]], fread(path), envir = .GlobalEnv)
    next
  }
...
if (length(read_counts_list) > 0) {
  panel_read_counts <- rbindlist(read_counts_list, fill = TRUE)
}

panel_depth_list <- list()
...
for (path in files_path$V1) {
  ...
  if (grepl("-depth\\.tsv$", fname)) {
    panel_depth_list[[fname]] <- fread(path)
  }
}

if (length(panel_depth_list) > 0) {
  panel_depth <- rbindlist(panel_depth_list, fill = TRUE)
}
