...
blood_panel <- data.frame(fread("https://github.com/MRCIEU/dnam-lung-cancer-screening-panel//raw/refs/heads/master/panel-reduced.csv")) 
...
for (i in 1:nrow(blood_panel )) {
  # Define region
  region <- paste0("chr", blood_panel$chr[i], ":", blood_panel $start[i], "-", blood_panel $end[i])
