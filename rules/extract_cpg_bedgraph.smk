rule extract_cpg_bedgraph:
    input:
        bam=config['paths']['output'] + "/mark_duplicates/{sample}.markdup.bam",
        fasta=config['inputs']['fasta']
    output:
        temp(config['paths']['output'] + "/extract_cpg_bedgraph/{sample}_CpG.bedGraph.gz")
    params:
    ...
    shell:
	    ...
	    -o {params.out_dir}/extract_cpg_bedgraph/{wildcards.sample} \
            --minDepth 10 --maxVariantFrac 0.25 \
            --nOT 0,0,0,98 --nOB 0,0,3,0 --mergeContext \
            {input.fasta} {input.bam} --keepDupes
        gzip {params.out_dir}/extract_cpg_bedgraph/{wildcards.sample}_CpG.bedGraph
        """)
