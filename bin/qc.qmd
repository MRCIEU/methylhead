...
label_map <- list(
  ...
  "estimate_cell_counts.csv"       = "estimate_cell_count",
  "Methylation_matrix.csv"         = "full_matrix",
...
full_matrix$loc <- paste(full_matrix$chr, full_matrix$start)

rownames(full_matrix) <- full_matrix$loc
...
if (params$dataset == "panel") {
  ...
  panel <- merge(panel, full_matrix, by = "loc")
  ...
  input.data <- full_matrix[, reads$Sample] 
}

input.coverage <- coverage_matrix[rownames(input.data), reads$Sample]

rm(coverage_matrix,full_matrix)
```
